//! Screen-space **god rays** (radial light scattering) post pass — the crepuscular-ray effect,
//! ported from Warbell (`src/godrays.rs` there; shader copied verbatim). This pass marches toward
//! the sun's SCREEN position, scattering the scene's own brightness into shafts along the treeline
//! silhouette. It's independent of fog and always visible when the sun is up and in/near frame.
//!
//! Same shape as [`crate::atmospherics`] / [`crate::dof`] — a Bevy 0.19 `Core3d`-schedule render
//! system (not a `ViewNode`) — but with NO prepass binding (the light mask is scene luminance, not
//! depth). Adaptations from Warbell: uses [`crate::sky::Sun`] as the sun marker, no game-state /
//! quality-preset / biome-mood coupling, `WED_GODRAYS` env override (was `FOREST_GODRAYS`), and the
//! chain is re-pinned `.after(atmospherics_pass).before(dof_pass)` (Warbell chained through an
//! `outline` pass that doesn't exist here). Pinned chain: `smaa → atmospherics → godrays → dof`.
...
/// by [`drive_godrays`]; the rest are the authored look (overridable via `WED_GODRAYS`).
...
pub struct GodRays {
    ...
    /// March step count.
    pub num_samples: f32,
    /// 0..1 master gate (daylight × on-screen alignment × settings gate), driven per-frame. 0 = no rays.
    pub fade: f32,
}
...
/// The authored default, with a `WED_GODRAYS="intensity,decay,density,weight,threshold,samples"`
...
pub fn default_godrays() -> GodRays {
    ...
    if let Ok(s) = std::env::var("WED_GODRAYS") {
...
/// Panel toggle + strength (scales the driven `fade`), mirroring [`crate::atmospherics::AtmoSettings`].
#[derive(Resource)]
...
impl Plugin for GodRaysPlugin {
    fn build(&self, app: &mut App) {
        ...
        // smaa → atmospherics → godrays → dof. Every post_process_write ping-pong MUST be
        // explicitly ordered or the multithreaded executor races them → flicker.
        render_app.add_systems(
            Core3d,
            godrays_pass
                .in_set(Core3dSystems::PostProcess)
                .after(smaa)
                .after(crate::atmospherics::atmospherics_pass)
...
/// `GodRays` component before extract). The sun is a directional light whose translation IS the
/// direction to the sun (see `sky.rs::setup_sun`), so its normalized translation is the sun dir.
fn drive_godrays(
    settings: Res<GodRaySettings>,
    ...
    // Panel gate: 0 when disabled, else the strength multiplier (mirrors AtmoSettings).
    let gate = if settings.enabled { settings.strength } else { 0.0 };
    ...
    for (cam_tf, cam, mut gr) in &mut cams {
        ...
        gr.fade = daylight * onscreen * gate;
...
/// so no explicit on/off guard is needed.
pub(crate) fn godrays_pass(
    ...
    let time_span = diagnostics.time_span(ctx.command_encoder(), "godrays");
