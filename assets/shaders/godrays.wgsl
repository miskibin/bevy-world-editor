// Screen-space **god rays** (radial light scattering) — the crepuscular-ray pass ported from
// Warbell (GPU Gems 3, Ch. 13). For each pixel we march N samples toward the sun's SCREEN
...
// occlusion buffer, no dependence on volumetric fog. Runs in PostProcess after atmospherics, so it
// scatters the already-graded + hazed image.
